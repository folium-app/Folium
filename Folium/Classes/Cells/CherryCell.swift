...
class CherryCell : GameCell {
    var game: CherryGame? = nil
    override func configureCell<T>(with game: T, controller: GamesController) {
        self.game = game as? CherryGame
        guard let game: CherryGame = self.game else {
            return
        }
...
extension CherryCell : UIImagePickerControllerDelegate, UINavigationControllerDelegate {
