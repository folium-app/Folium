...
class CytrusCell : GameCell {
    var game: CytrusGame? = nil
    override func configureCell<T>(with game: T, controller: GamesController) {
        self.game = game as? CytrusGame
        guard let game: CytrusGame = self.game else {
            return
        }
        ...
        } else if let image: UIImage = (game.boxart as Data).decodeRGB565(width: 48, height: 48) {
...
extension CytrusCell : UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        _  = Task {
            do {
                ...
                imageView.image = if let image: UIImage = (game.boxart as Data).decodeRGB565(width: 48, height: 48) {
                    image
                } else {
                    nil
                }
