...
class KiwiCell : GameCell {
    var game: KiwiGame? = nil
    override func configureCell<T>(with game: T, controller: GamesController) {
        self.game = game as? KiwiGame
        guard let game: KiwiGame = self.game else {
            return
        }
...
extension KiwiCell : UIImagePickerControllerDelegate, UINavigationControllerDelegate {
