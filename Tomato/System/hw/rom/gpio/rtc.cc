...
void RTC::Write(int value) {
  ...
  if(GetPortDirection(static_cast<int>(Port::CS)) == PortDirection::Out) {
    ...
    printf("RTC: CS port should be set to 'output' but configured as 'input'.\n");;
  }

  if(GetPortDirection(static_cast<int>(Port::SCK)) == PortDirection::Out) {
    port.sck = (value >> static_cast<int>(Port::SCK)) & 1;
  } else {
      printf("RTC: SCK port should be set to 'output' but configured as 'input'.\n");
  }
...
void RTC::ReceiveCommandSIO() {
  ...
  if((data >> 4) == 6) {
    ...
    data = ((data & 0x55) << 1) | ((data & 0xAA) >> 1);
      ...
      printf("RTC: received command in unknown format, data=0x%x\n", data);
    return;
  }
  ...
  switch(reg) {
    case Register::Control: {
      ...
      if(control.per_minute_irq) {
          printf("RTC: enabled the unimplemented per-minute IRQ.\n");
      }
      break;
    }
    ...
    default: {
        printf("RTC: unhandled register write: %i\n", (int)reg);
      break;
    }
  }
}
