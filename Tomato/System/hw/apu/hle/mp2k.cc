...
#include "nba/bus/bus.hh"
#include "nba/hw/apu/hle/mp2k.hh"
...
void MP2K::SoundMainRAM(SoundInfo const& sound_info) {
  ...
  if(!engaged) {
    if(sound_info.pcm_samples_per_vblank == 0) [[unlikely]] {
      printf("Samples per V-blank must not be zero.\n");
    }
  ...
  for(int i = 0; i < max_channels; i++) {
    ...
    if(channel.status & CHANNEL_START) {
      ...
      if(wave_info == nullptr) {
        printf("MP2K: channel[{}] wave address is invalid: 0x%x\n", channel.wave_address);
  ...
  for(int i = 0; i < max_channels; i++) {
    ...
    if(sampler.compressed != compressed || !sampler.wave_data) {
      ...
      if(sampler.wave_data == nullptr) {
        printf("MP2K: channel[%i] sample data has bad memory range 0x%x - 0x%x.\n", i, wave_data_begin, wave_data_begin + wave_size);
        channel.status = 0; // Disable channel, there is no good way to deal with this.
        continue;
      }
